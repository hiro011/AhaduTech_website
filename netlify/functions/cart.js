// netlify/functions/cart.js – FINAL VERSION (works with your "image" column)
...
export default async (req) => {
  ...
  try {
    ...
    // Helper: get full cart with product details
    const getCart = async () => {
      return await sql`
        SELECT 
          ci.id,
    ...
    if (action === 'add') {
      ...
      if (newQty > product[0].stock) {
        return new Response(JSON.stringify({ error: `Only ${product[0].stock} left in stock` }), { status: 400 });
      }
